...
void TileMap::priv_updateVertices() const
{
	...
	const sf::Vector2f actualCamera{ priv_getActualCamera() };
	const sf::Vector2f cameraOffset{ trunc((actualCamera.x - std::floor(actualCamera.x)) * m_textureTileSize.x), trunc((actualCamera.y - std::floor(actualCamera.y)) * m_textureTileSize.y) };
...
void TileMap::priv_updateRender() const
{
	...
	if (m_do.scrollSmoothly)
	{
		...
		const sf::Vector2f fractionOffset{ round((cameraOffset.x - std::floor(cameraOffset.x)) * pixelSize.x), round((cameraOffset.y - std::floor(cameraOffset.y)) * pixelSize.y) };
